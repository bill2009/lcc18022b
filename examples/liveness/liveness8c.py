#version 8.cattempt to process by routine - not very effective
from __future__ import print_function
print ("Python Liveness analysis 8b - full syntax - revamped finale")
retops=["cretn"]
callops=["ccall"]
...
routines=dict() # named c functions
fname=r"C:\cygwin\tmp/lcc213441" #"testrockets" #"reduced_full_lcc1802_combo"#"test2"
def loadpgm():
        ...
        if True:
            ...
            if src.strip()=="":
                if pgm[n].startswith(";;function_start"):                
                    ...
                    routines[pgm[n].split(' ')[1]]=[n]
                elif pgm[n].startswith(";;function_end"): #;;function_end IO1802.inc
                    routines[pgm[n].split(' ')[1]].append(n)
                continue
            if src.find(":")>0:
                label[n]=src.split(":")[0]
                targets[label[n]]=n
...
print("***********")
for routine in ["_handle_horizontal_missiles"]:
    begin=routines[routine][0] #first line
    end=routines[routine][1] #last line
