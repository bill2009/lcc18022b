...
def loadpgm():
    ...
    if fname=="":
        fname="test1" #"reduced_full_lcc1802_combo"
    fname=fname+".asm"
    fhand=open(fname)
    for line in fhand:
    ...
    for n in range(len(pgm)):
        ...
        if src.strip()=="":
            if pgm[n].startswith(";$$function_start$$"):
                functype=pgm[n].split("=")[1].split("(")[0] # isolate the function type
                print functype,"**",pgm[n]
            continue
        labelq=re.findall("^([\S]+):",src)
        if len(labelq)==1:
            print "labelq:",labelq[0]
        if src.find(":")>0:
            label[n]=src.split(":")[0]
        ...
        if opcode[n] in IUs: #immed, used reg
            ...
            uses[n]=[ops[0],ops[1]]; refs[n]=[ops[2]]
            ...
            print "ignoring",opcode[n]," on line ",n
...
for n in range(len(pgm)-1):
    if opcode[n] in cjumps0+cjumps1+cjumps2+cjumps3+cjumps4:
        print "**************CJUMP*************",n
