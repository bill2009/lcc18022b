...
//20-05-22 rearranged dec/inc in memset
 int16_t strncmp(const char *s1, const char *s2, uint16_t n)
/* ANSI sez:
...
char * ftoa(float flt, char *outbfr,uint16_t dec_digits)
{
#define use_leading_plus 0
	uint16_t idx;
	float mult;
	int32_t wholeNum,decimalNum;
   char *output = outbfr ;
   char tbfr[40] ;
   //*******************************************
...
void *memset(void *s, int16_t c, uint16_t n) //sets memory at s to c for n bytes
{
    unsigned char* p=s;
    while(n){
        *p = (unsigned char)c;
