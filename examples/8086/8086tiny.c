...
#define IO_PORT_COUNT 256
#define RAM_SIZE 1024 //was 10fff0
...
#define R_M_OP(dest,op,src) (i_w ? op_dest = CAST(unsigned int)dest, op_result = CAST(unsigned int)dest op (op_source = CAST(unsigned int)src) \
...
#define SEGREG(reg_seg,reg_ofs,op) 16 * regs16[reg_seg] + (unsigned int)(op regs16[reg_ofs])

// Returns sign bit of an 8-bit or 16-bit operand
#define SIGN_OF(a) (1 & (i_w ? CAST(int)a : a) >> (TOP_BIT - 1))
...
#else
#define SDL_KEYBOARD_DRIVER sdl_screen ? SDL_PollEvent(&sdl_event) && (sdl_event.type == SDL_KEYDOWN || sdl_event.type == SDL_KEYUP) && (scratch_uint = sdl_event.key.keysym.unicode, scratch2_uint = sdl_event.key.keysym.mod, CAST(int)mem[0x4A6] = 0x400 + 0x800*!!(scratch2_uint & KMOD_ALT) + 0x1000*!!(scratch2_uint & KMOD_SHIFT) + 0x2000*!!(scratch2_uint & KMOD_CTRL) + 0x4000*(sdl_event.type == SDL_KEYUP) + ((!scratch_uint || scratch_uint > 0x7F) ? sdl_event.key.keysym.sym : scratch_uint), pc_interrupt(7)) : (KEYBOARD_DRIVER)
#endif

// Global variable definitions
unsigned char mem[RAM_SIZE];
unsigned char io_ports[IO_PORT_COUNT];
unsigned char *opcode_stream, *regs8, i_rm, i_w, i_reg, i_mod, i_mod_size, i_d, i_reg4bit, raw_opcode_id, xlat_opcode_id, extra, rep_mode, seg_override_en, rep_override_en, trap_flag, int8_asap, scratch_uchar, io_hi_lo, *vid_mem_base, spkr_en, bios_table_lookup[2][256]; //was [20][256]
//unsigned char mem[RAM_SIZE], io_ports[IO_PORT_COUNT], *opcode_stream, *regs8, i_rm, i_w, i_reg, i_mod, i_mod_size, i_d, i_reg4bit, raw_opcode_id, xlat_opcode_id, extra, rep_mode, seg_override_en, rep_override_en, trap_flag, int8_asap, scratch_uchar, io_hi_lo, *vid_mem_base, spkr_en, bios_table_lookup[2][256]; //was [20][256]
unsigned int *regs16, reg_ip, seg_override, file_index, wave_counter;
...
SDL_Event sdl_event;
unsigned int vid_addr_lookup[VIDEO_RAM_SIZE], cga_colors[4] = {0 /* Black */, 0x1F1F /* Cyan */, 0xE3E3 /* Magenta */, 0xFFFF /* White */};
#endif
...
{
	for (int i = 0; i < len; i++)
		stream[i] = (spkr_en == 3) && CAST(unsigned int)mem[0x4AA] ? -((54 * wave_counter++ / CAST(unsigned int)mem[0x4AA]) & 1) : sdl_audio.silence;
...
int main(int argc, char **argv)
{
	...
	regs16 = (unsigned int *)(regs8 = mem + REGS_BASE);
	...
	for (; opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip, opcode_stream != mem;)
	{
		...
		i_data0 = CAST(int)opcode_stream[1];
		i_data1 = CAST(int)opcode_stream[2];
		i_data2 = CAST(int)opcode_stream[3];
		...
		if (i_mod_size)
		{
			...
			if ((!i_mod && i_rm == 6) || (i_mod == 2))
				i_data2 = CAST(int)opcode_stream[4];
			else if (i_mod != 1)
				i_data2 = i_data1;
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE_CHAIN 5: // INC|DEC|JMP|CALL|PUSH
				if (i_reg < 2) // INC|DEC
					...
					i_reg & 1 && (regs16[REG_CS] = CAST(int)mem[op_from_addr + 2]), // JMP|CALL (far)
			...
			OPCODE 6: // TEST r/m, imm16 / NOT|NEG|MUL|IMUL|DIV|IDIV reg
				...
				switch (i_reg)
				{
					...
					OPCODE 4: // MUL
						i_w ? MUL_MACRO(unsigned int, regs16) : MUL_MACRO(unsigned char, regs8)
					OPCODE 5: // IMUL
						i_w ? MUL_MACRO(int, regs16) : MUL_MACRO(char, regs8)
					OPCODE 6: // DIV
						i_w ? DIV_MACRO(unsigned int, unsigned, regs16) : DIV_MACRO(unsigned char, unsigned int, regs8)
					OPCODE 7: // IDIV
						i_w ? DIV_MACRO(int, int, regs16) : DIV_MACRO(char, int, regs8);
				}
			...
			OPCODE 21: // IN AL/AX, DX/imm8
				...
				scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 7) && (io_ports[0x3D5] = ((mem[0x49E]*80 + mem[0x49D] + CAST(int)mem[0x4AD]) & (io_ports[0x3D4] & 1 ? 0xFF : 0xFF00)) >> (io_ports[0x3D4] & 1 ? 0 : 8)); // CRT cursor position
			...
			OPCODE 22: // OUT DX/imm8, AL/AX
				...
				scratch_uint == 0x3D5 && (io_ports[0x3D4] >> 1 == 7) && (scratch2_uint = ((mem[0x49E]*80 + mem[0x49D] + CAST(int)mem[0x4AD]) & (io_ports[0x3D4] & 1 ? 0xFF00 : 0xFF)) + (regs8[REG_AL] << (io_ports[0x3D4] & 1 ? 0 : 8)) - CAST(int)mem[0x4AD], mem[0x49D] = scratch2_uint % 80, mem[0x49E] = scratch2_uint / 80); // CRT cursor position
			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					...
					OPCODE 1: // GET_RTC
						...
						CAST(int)mem[SEGREG(REG_ES, REG_BX, 36+)] = ms_clock.millitm;
					OPCODE 2: // DISK_READ
