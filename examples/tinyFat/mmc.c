...
*/
#include "mmc.h"
//#include <hspi2.h> //commented out for bitbang - moved to caller
#include <olduino.h>
//#define spiRec() spixfer(0xff) //commented out for bitbang - moved to caller
...
uint8_t mmc_cardCommand(uint8_t cmd, uint32_t arg)
{
	...
	for (i = 0; ((status_ = spiRec()) & 0X80) && i != 0XFF; i++);// printf("%d.",status_);
	//status_=spiRec();
	//printf("%d-",status_);
	//status_=spiRec();
	//printf("%d.",status_);
...
{
	uint16_t t0 =128; //explicit cycle count
	uint32_t arg;
	uint8_t status_,i;
	...
	{
		printf("%d,%d ",status_,STATUS_IN_IDLE);
		if (!(t0--))
		{
	...
	arg = mmc__card_type == SD_CARD_TYPE_SD2 ? 0X40000000 : 0; //was just _card...
	//arg=0; //wjr 18-03-17
	...
	return 0;//wjr so i can tell it failed
...
byte mmc_readSector(byte *buffer, uint32_t sector)
{
	...
	uint16_t i;
	printf("\nmmc_readSector card type %d sector passed=0x%lx...",mmc__card_type,sector);
	if (mmc__card_type != SD_CARD_TYPE_SDHC)	sector <<= 9;
	printf("single block read=0x%lx\n",sector);
	tries=0;
