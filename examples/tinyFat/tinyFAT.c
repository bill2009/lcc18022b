...
void le2int(unsigned int * i, unsigned char * c){
	asm(" inc 12 ; point to end of int\n"
		" lda 13 ; pick up low order byte\n"
		" str 12 ; store it\n"
		" ldn 13 ; high byte\n"
		" dec 12 ; back up\n"
		" str 12 ; store high byte\n");
}
	...
	if (0==rc){
		...
		if (RES_OK == mmc_readSector(buffer, 0))
		{
			if ((buffer[0x01FE]==0x55) && (buffer[0x01FF]==0xAA))
			{
				...
				dump(buffer,512); //wjr 18-03-17
				printf("MBR.part1Type=%cx\n",MBR.part1Type);
			}
			else
			{
				dump(buffer,512); //wjr 18-03-17
		...
		if ((MBR.part1Type!=0x04) && (MBR.part1Type!=0x06) && (MBR.part1Type!=0x86)&& (MBR.part1Type!=0x0E))//wjr18-03-17 added 0E
		{
	...
	// Read Boot Sector
		if (RES_OK == mmc_readSector(buffer, MBR.part1Start))
		{
			if ((buffer[0x01FE]==0x55) && (buffer[0x01FF]==0xAA))
			{
				...
				le2int(&BS.reservedSectors,buffer+0x0e);
