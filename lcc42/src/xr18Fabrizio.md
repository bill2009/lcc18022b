 ...
 * 20-11-20 mucking with global initializations xr18fabrizio, trying to copy inits from rom to ram
...
stmt: LTI2(CVUI2(reg),CVUI2(reg))  "\tjcU1 R%0,R%1,lbnf,%a;  i.e. subtract B from A and jump if borrow **opt 20**\n"   1
stmt: LTI2(CVUI2(reg),SUBI2(CVUI2(reg),consm))  "\tld1 R14,R%1\n\tdecm R14,%2\n\tjcU1 R%0,R14,lbnf,%a ;LT=lbnf opt20-B\n"  42
...
static void progbeg(int argc, char *argv[]) {
	...
	fprintf(stderr,"Still Storming In September!\n");  //just so I know who's playing
        time(&now);
...
static void defconst(int suffix, int size, Value v) {
    ...
    if (cseg==3){/* initialized global*?*/
	print("\torgc ;for initializers\n");
        if (suffix == F && size == 4) {
                float f = v.d;
                print("\tdd 0x%x\n", *(unsigned *)&f);
        }        else if (suffix == P)
...
static void global(Symbol p) {
	int dbsize;
		...
		printf(";**	non-bss global 20-11-20\n");
	}       	
}
