...
Type promote(Type ty) {
	fprintf(stderr, "Promoting type with op %i and size %i(inttype->op is %i size is %i)\n",ty->op,ty->size,inttype->op, inttype->size);
		fprintf(stderr, "  rogue promotion bypass returning unsignedchar ty->op=%d, ty->size=%d\n",unsignedchar->op,unsignedchar->size);
		return unsignedchar;
	/*if (ty==chartype || ty==unsignedchar){
		fprintf(stderr, "  rogue promotion bypass returning ty->op=%d, ty->size=%d\n",ty->op,ty->size);
		return ty;
	}*/
	ty = unqual(ty);
	switch (ty->op) {
	case ENUM:
		fprintf(stderr, "  ENUM Returning inttype\n");
		return inttype;
	case INT:
		if (ty->size < inttype->size){
			fprintf(stderr, "  smaller INT Returning inttype\n");
			return inttype;
		}
		break;
	case UNSIGNED:
		if (ty->size < inttype->size){
			fprintf(stderr, "  smaller UNSIGNED Returning inttype\n");
			return inttype;
		}
		if (ty->size < unsignedtype->size){
			fprintf(stderr, "  UNSIGNED Returning unsignedtype\n");
			return unsignedtype;
		}
		break;
	case FLOAT:
		if (ty->size < doubletype->size){
			fprintf(stderr, "  FLOAT Returning doubletype\n");
			return doubletype;
		}
	}
	fprintf(stderr, "  otherwise Returning ty->op=%i, ty->size=%i\n",ty->op,ty->size);
	return ty;
}
...
#define xx(ty) if (size == (ty)->size) return ty;
	fprintf(stderr,"btot(%d,%d) optype=%d, F=%d,I=%d,U=%d,P=%d\n",op,size,optype(op),F,I,U,P);
	switch (optype(op)) {
	case F:
		xx(floattype);
		xx(doubletype);
		...
		assert(0); return 0;
	case I:
	//fprintf(stderr,"case I: chartype->op=%d, INT=%d, chartype->size=%d\n",chartype->op,INT,chartype->size);
		if (chartype->op == INT)
			xx(chartype);
		xx(signedchar);
		xx(shorttype);
		...
		xx(longtype);
		xx(longlong);
		//fprintf(stderr,"trying unsignedlong->size=%d\n",unsignedlong->size);
		/*xx(unsignedlong)*/ //if (size == unsignedlong->size) return unsignedlong; //wjr
		assert(0); return 0;
	case U:
